...
async function main() {
  try {
    ...
    const upgrade = new UserMessage(`
      ...
      3. 价格方案如何？
    `);
    const info = await customerService.run(upgrade);
